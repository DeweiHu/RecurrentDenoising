# -*- coding: utf-8 -*-
"""
Created on Thu Oct 15 05:32:25 2020
...
model = arch.res_UNet(n_seq,enc_nch).to(device)
...
for epoch in range(n_epoch):
    ...
    for step,(x_seq,y_seq) in enumerate(train_loader):
        ...
        x = Variable(x_seq).to(device)
        y = Variable(y_seq[:,-1,:,:,:]).to(device)    
        y_pred = model(x)
        ...
        if step == len(train_loader)-1:
            pred = util.ImageRescale(y_pred[0,-1,:,:500].detach().cpu().numpy(),[0,255])
